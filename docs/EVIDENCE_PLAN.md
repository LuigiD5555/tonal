...
> **Semana 1 — Validar el ledger de Tlaloc y generar su tabla de estado.**
>
> Tomar `tlaloc/state/CLAIMS.json` como única fuente de claims de Tlaloc.
> Añadir un validador determinista que compruebe esquema, IDs únicos, estados permitidos, evidencia obligatoria para `implemented` o superior y existencia de cada referencia de test.
> Generar la tabla de `docs/CAPABILITY_STATUS.md` desde el ledger y hacer que CI rechace divergencia o edición manual.
> Fuentes permitidas: el ledger, código real, tests existentes y los scripts de verificación actuales.
> No añadir claims nuevos ni avanzar a Run Records en esta tarea.
