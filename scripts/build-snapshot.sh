...
cp VERSION TONAL.json tonal.lock PROJECT_BOUNDARY.md .work/snapshot/
mkdir -p .work/snapshot/skills; cp -a skills/. .work/snapshot/skills/
version=$(cat VERSION)
archive="dist/tonal-${version}-source.tar.gz"
TZ=UTC tar --sort=name --mtime='UTC 1970-01-01' --owner=0 --group=0 --numeric-owner -C .work/snapshot -czf "$archive" .
